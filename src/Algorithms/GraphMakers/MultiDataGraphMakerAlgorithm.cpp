...
MultiDataGraphMakerAlgorithm::MultiDataGraphMakerAlgorithm(
    const std::string &name, const Config &cfg)
    : Core::BaseAlgorithm(name), m_cfg(cfg) {}

void MultiDataGraphMakerAlgorithm::initialize() {
  ...
  if (m_cfg.inputMaskName.empty())
    throw std::invalid_argument("Missing input mask name for algorithm " +
                                name());
  ...
  if (m_cfg.variableNames.size() != m_cfg.graphDefs.size())
    throw std::invalid_argument(
        "Mismatch between variables and graph definitions for algorithm " +
        name());
}

void MultiDataGraphMakerAlgorithm::execute(Core::EventContext &context) {
  ...
  for (auto igr(0); igr < nSize; igr++) {
    const std::pair<std::string, std::string> &var_names =
        m_cfg.variableNames.at(igr);
    auto &gr = m_cfg.graphDefs.at(igr);

    std::visit(
        [&](auto &h_to_add) {
          const std::string grName(var_names.first + "_" + var_names.second);
          EventDataModel::GraphObject toAdd("gr_" + grName, var_names.first,
                                            var_names.second, h_to_add);
