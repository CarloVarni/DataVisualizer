...
SeedingPerformanceRootReaderAlgorithm::SeedingPerformanceRootReaderAlgorithm(
    const std::string &name,
    const Algorithm::SeedingPerformanceRootReaderAlgorithm::Config &cfg)
    : RootFileReaderAlgorithm(name, cfg) {}

void SeedingPerformanceRootReaderAlgorithm::execute(
  ...
  std::vector<EventDataModel::MultiDataObjectCollection::value_type::value_type>
      values;
  ...
  context.add(m_cfg.dataCollectionName, std::move(outputCollection));
  context.add(m_cfg.outputMaskName, std::move(mask));
}
