...
DataHistogramMakerAlgorithm::DataHistogramMakerAlgorithm(
    const std::string &name, const Config &cfg)
    : Core::BaseAlgorithm(name), m_cfg(cfg) {}

void DataHistogramMakerAlgorithm::initialize() {
...
void DataHistogramMakerAlgorithm::execute(Core::EventContext &context) {

  MSG_INFO("Executing " + name() + " ... ");

  const auto dataCollection =
      context.get<EventDataModel::DataObjectCollection>(m_cfg.inputCollection);
  ...
  for (auto ihisto(0); ihisto < nSize_1D; ihisto++) {
    ...
    std::visit(
        [&](auto &h_to_add) {
          EventDataModel::HistogramObject_1D toAdd("h_" + var_name, var_name,
                                                   h_to_add);
