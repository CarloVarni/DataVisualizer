...
MultiDataHistogramMakerAlgorithm::MultiDataHistogramMakerAlgorithm(
    const std::string &name, const Config &cfg)
    : Core::BaseAlgorithm(name), m_cfg(cfg) {}

void MultiDataHistogramMakerAlgorithm::initialize() {
  ...
  if (m_cfg.variableNames_1D.size() != m_cfg.histogramDefs_1D.size())
    throw std::invalid_argument(
        "Mismatch between size of 1D variables and histograms for algorithm " +
        name());

  if (m_cfg.variableNames_2D.size() != m_cfg.histogramDefs_2D.size())
    throw std::invalid_argument(
        "Mismatch between size of 2D variables and histograms for algorithm " +
        name());

  if (m_cfg.outputCollection_1d.empty() and m_cfg.outputCollection_2d.empty())
    throw std::invalid_argument(
        "Both 1d and 2d output collections are missing for algorithm " +
        name());

  if (m_cfg.outputCollection_1d.empty() and m_cfg.variableNames_1D.size() != 0)
    throw std::invalid_argument("Output collection for 1D histograms cannot be "
                                "empty if we are producing them");

  if (m_cfg.outputCollection_2d.empty() and m_cfg.variableNames_2D.size() != 0)
    throw std::invalid_argument("Output collection for 2D histograms cannot be "
                                "empty if we are producing them");
}

void MultiDataHistogramMakerAlgorithm::execute(Core::EventContext &context) {

  MSG_INFO("Executing " + name() + " ... ");

  const auto dataCollection =
      context.get<EventDataModel::MultiDataObjectCollection>(
          m_cfg.inputCollection);
  MSG_INFO("Retrieved " + std::to_string(dataCollection->size()) + " objects");
  if (dataCollection->size() == 0) {
    MSG_FATAL(
        "No input data have been found. Cannot create histogram from that!!!");
  }
  ...
  for (auto ihisto(0); ihisto < nSize_1D; ihisto++) {
    ...
    std::visit(
        [&](auto &h_to_add) {
          ...
          EventDataModel::HistogramObject_1D toAdd("h_" + histoName, var_name,
                                                   h_to_add);
  ...
  for (auto ihisto(0); ihisto < nSize_2D; ihisto++) {
    ...
    std::visit(
        [&](auto &h_to_add) {
          const std::string histoName(h_to_add.GetName());
          EventDataModel::HistogramObject_2D toAdd("h_" + histoName, var_x_name,
                                                   var_y_name, h_to_add);
          map_2D->emplace("h_" + var_x_name + "_" + var_y_name,
                          histograms_2D->size());
