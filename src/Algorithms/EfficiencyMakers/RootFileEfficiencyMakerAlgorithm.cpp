...
RootFileEfficiencyMakerAlgorithm::RootFileEfficiencyMakerAlgorithm(
    const std::string &name, const Config &cfg)
    : Core::BaseAlgorithm(name), m_cfg(cfg) {}

void RootFileEfficiencyMakerAlgorithm::initialize() {
  ...
  if (m_cfg.output_collection_1d.empty())
    throw std::invalid_argument(
        "Missing 1D output collection name for algorithm " + name());

  if (m_cfg.output_collection_2d.empty())
    throw std::invalid_argument(
        "Missing 2D output collection name for algorithm " + name());
}

void RootFileEfficiencyMakerAlgorithm::execute(Core::EventContext &context) {
  MSG_INFO("Executing " + name() + " ...");

  std::shared_ptr<TFile> file =
      std::make_shared<TFile>(m_cfg.inputFile.c_str(), "READ");

  std::pair<std::shared_ptr<EventDataModel::EfficiencyObjectCollection>,
  ...
  if (not m_cfg.output_collection_1d.empty()) {
    context.add("eff_1d_" + m_cfg.output_collection_1d,
                std::move(efficiency_collections.first));
    context.add("index_eff_1d_" + m_cfg.output_collection_1d,
                std::move(efficiency_names_1d));
  }

  if (not m_cfg.output_collection_2d.empty()) {
    context.add("eff_2d_" + m_cfg.output_collection_2d,
                std::move(efficiency_collections.second));
    context.add("index_eff_2d_" + m_cfg.output_collection_2d,
                std::move(efficiency_names_2d));
  }
