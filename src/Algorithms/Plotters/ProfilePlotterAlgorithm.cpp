...
ProfilePlotterAlgorithm::ProfilePlotterAlgorithm(
    const std::string &name,
    const Algorithm::PlotterAlgorithm<
        EventDataModel::ProfileObjectCollection>::Config &cfg)
    : PlotterAlgorithm(name, cfg) {}

void ProfilePlotterAlgorithm::DrawCollection(
    const EventDataModel::ProfileObjectCollection *collection) {

  for (const auto &object : *collection) {
    auto &object_to_draw =
        *const_cast<EventDataModel::ProfileObject *>(&object);
