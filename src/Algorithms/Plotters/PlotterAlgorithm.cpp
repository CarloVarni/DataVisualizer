...
PlotterAlgorithm::PlotterAlgorithm(const std::string &name, const Config &cfg)
    : Core::BaseAlgorithm(name), m_cfg(cfg) {}

void PlotterAlgorithm::initialize() {
  ...
  if (m_cfg.inputCollection_hist_1d.empty() and
      ...
      m_cfg.inputCollection_gr.empty() and m_cfg.inputCollection_prof.empty())
    throw std::invalid_argument(name() +
                                " requires an input collection to be defined!");

  if (m_cfg.outputFolder.empty())
    throw std::invalid_argument(
        name() + " requires an output folder for the plots to be specified!");
}

void PlotterAlgorithm::execute(Core::EventContext &context) {
  ...
  if (not m_cfg.inputCollection_hist_1d.empty())
    histograms_1D = context.get<EventDataModel::HistogramObjectCollection_1D>(
        "h_1d_" + m_cfg.inputCollection_hist_1d);

  const EventDataModel::HistogramObjectCollection_2D *histograms_2D = nullptr;
  if (not m_cfg.inputCollection_hist_2d.empty())
    histograms_2D = context.get<EventDataModel::HistogramObjectCollection_2D>(
        "h_2d_" + m_cfg.inputCollection_hist_2d);

  const EventDataModel::EfficiencyObjectCollection *efficiency_1D = nullptr;
  if (not m_cfg.inputCollection_eff_1d.empty())
    efficiency_1D = context.get<EventDataModel::EfficiencyObjectCollection>(
        "eff_1d_" + m_cfg.inputCollection_eff_1d);

  const EventDataModel::EfficiencyObjectCollection *efficiency_2D = nullptr;
  if (not m_cfg.inputCollection_eff_2d.empty())
    efficiency_2D = context.get<EventDataModel::EfficiencyObjectCollection>(
        "eff_2d_" + m_cfg.inputCollection_eff_2d);

  const EventDataModel::GraphObjectCollection *graphs = nullptr;
  if (not m_cfg.inputCollection_gr.empty())
    graphs = context.get<EventDataModel::GraphObjectCollection>(
        "gr_" + m_cfg.inputCollection_gr);

  const EventDataModel::ProfileObjectCollection *profiles = nullptr;
  if (not m_cfg.inputCollection_prof.empty())
    profiles = context.get<EventDataModel::ProfileObjectCollection>(
        "prof_" + m_cfg.inputCollection_prof);

  // Histograms 1D
  if (histograms_1D) {
    for (const auto &const_histo : *histograms_1D) {
      auto &histo =
          *const_cast<EventDataModel::HistogramObject_1D *>(&const_histo);
      const std::string saving_name =
          m_cfg.outputFolder + "/" + histo.title() + ".pdf";
  ...
  if (histograms_2D) {
    for (const auto &const_histo : *histograms_2D) {
      auto &histo =
          *const_cast<EventDataModel::HistogramObject_2D *>(&const_histo);
      const std::string saving_name =
          m_cfg.outputFolder + "/" + histo.title() + ".pdf";
  ...
  if (efficiency_1D) {
    for (const auto &const_eff : *efficiency_1D) {
      ...
      const std::string saving_name =
          m_cfg.outputFolder + "/" + eff.title() + ".pdf";
  ...
  if (graphs) {
    for (const auto &const_gr : *graphs) {
      ...
      const std::string saving_name =
          m_cfg.outputFolder + "/" + gr.title() + ".pdf";
  ...
  if (profiles) {
    for (const auto &const_prof : *profiles) {
      ...
      const std::string saving_name =
          m_cfg.outputFolder + "/" + prof.title() + ".pdf";
