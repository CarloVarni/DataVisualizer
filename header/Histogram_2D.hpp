...
public:
  ...
  template<typename = std::enable_if_t<std::is_base_of<TH2, histogram_t>::value>>
...
template<typename data_x_t,
	 typename data_y_t,
         typename histogram_t>
template<typename>
Histogram_2D<data_x_t, data_y_t, histogram_t>::Histogram_2D(const std::string& title,
							    int nbins_x, double min_x, double max_x,
							    int nbins_y, double min_y, double max_y)
  : m_title(title),
    m_target_variable_x(""),
...
template<typename ... args>
void
Histogram_2D<data_x_t, data_y_t, histogram_t>::Fill(const MultiData<args...>& data)
{
...
template<typename data_x_t,
         ...
         typename histogram_t>
void
Histogram_2D<data_x_t, data_y_t, histogram_t>::Draw(TCanvas& canvas,
						    const std::string& option)
{
  canvas.cd();
...
template<typename fill_x_t,
	 typename fill_y_t>
void
Histogram_2D<data_x_t, data_y_t, histogram_t>::Fill(const fill_x_t value_x,
						    const fill_y_t value_y,
						    const float weight )
{
