...
public:
  ...
  template<typename = std::enable_if_t<not std::is_base_of<TH2, histogram_t>::value>>
...
template<typename data_t,
         typename histogram_t>
template<typename>
Histogram_1D<data_t, histogram_t>::Histogram_1D(const std::string& title,
                                                int nbins, double min, double max)
  : m_title(title),
    m_target_variable(""),
...
template<typename data_t,
	 typename histogram_t>
void
Histogram_1D<data_t, histogram_t>::Fill(const Data<data_t>& data)
...
template<typename ... args>
void
Histogram_1D<data_t, histogram_t>::Fill(const MultiData<args...>& data)
{
...
template<typename data_t,
         typename histogram_t>
void
Histogram_1D<data_t, histogram_t>::Draw(TCanvas& canvas,
					const std::string& option)
{
  canvas.cd();
...
template<typename fill_t>
void
Histogram_1D<data_t, histogram_t>::Fill(const fill_t value,
					const float weight )
{
