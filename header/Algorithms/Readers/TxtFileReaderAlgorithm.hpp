...
class TxtFileReaderAlgorithm : public Core::BaseAlgorithm {
public:
  ...
  static_assert(
      ...
      "TxtFileReaderAlgorithm only support DataObjectCollection and "
      "MultiDataObjectCollection output format");
  ...
  TxtFileReaderAlgorithm(const std::string &name, const Config &cfg);
...
template <typename data_t> void TxtFileReaderAlgorithm<data_t>::initialize() {
  ...
  if (m_cfg.outputMask.empty())
    throw std::invalid_argument("Missing output mask name for algorithm " +
                                name());
}

template <typename data_t>
void TxtFileReaderAlgorithm<data_t>::execute(Core::EventContext &context) {
  MSG_INFO("Executing " + name() + " ...");

  std::shared_ptr<data_t> data_collection = std::make_shared<data_t>();
  ...
  MSG_INFO("Retrieved " + std::to_string(data_collection->size()) +
           " data objects");
  input_file.close();
...
using TxtFileReaderToDataObjectCollectionAlgorithm =
    TxtFileReaderAlgorithm<EventDataModel::DataObjectCollection>;
using TXTFileReaderToMultiDataObjectCollectionAlgorithm =
    TxtFileReaderAlgorithm<EventDataModel::MultiDataObjectCollection>;
} // namespace Algorithm
