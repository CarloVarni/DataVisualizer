...
class HistogramComparisonPlotterAlgorithm
    : public ComparisonPlotterAlgorithm<
          EventDataModel::HistogramObjectCollection_1D> {
public:
  HistogramComparisonPlotterAlgorithm() = delete;
  HistogramComparisonPlotterAlgorithm(
      const std::string &name,
      const ComparisonPlotterAlgorithm<
          EventDataModel::HistogramObjectCollection_1D>::Config &cfg);
  HistogramComparisonPlotterAlgorithm(
      const HistogramComparisonPlotterAlgorithm &) = delete;
  HistogramComparisonPlotterAlgorithm &
  operator=(const HistogramComparisonPlotterAlgorithm &) = delete;
