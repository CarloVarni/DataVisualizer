...
public:
  ...
  Histogram2DPlotterAlgorithm(const std::string &name,
  ...
  Histogram2DPlotterAlgorithm &operator=(const Histogram2DPlotterAlgorithm &) = delete;
  virtual ~Histogram2DPlotterAlgorithm() = default;

protected:
  virtual void DrawCollection(const EventDataModel::HistogramObjectCollection_2D *collection);
};
