...
class ComparisonPlotterAlgorithm : public Core::BaseAlgorithm {
public:
  struct Config {
  ...
  ComparisonPlotterAlgorithm(const std::string &name, const Config &cfg);
  ComparisonPlotterAlgorithm(const ComparisonPlotterAlgorithm &) = delete;
  ComparisonPlotterAlgorithm &
  operator=(const ComparisonPlotterAlgorithm &) = delete;
...
template <typename object_collection_t>
ComparisonPlotterAlgorithm<object_collection_t>::ComparisonPlotterAlgorithm(
    const std::string &name, const Config &cfg)
    : Core::BaseAlgorithm(name), m_cfg(cfg) {}

template <typename object_collection_t>
void ComparisonPlotterAlgorithm<object_collection_t>::initialize() {
  ...
  if (m_cfg.variableNames.size() == 0)
    throw std::invalid_argument("Missing variable name list for algorithm " +
                                name());
}

template <typename object_collection_t>
void ComparisonPlotterAlgorithm<object_collection_t>::execute(
  ...
  for (const std::string &input : m_cfg.originalInputCollection) {
    object_collections.push_back(
        context.get<object_collection_t>(m_cfg.prefix + "_" + input));
    object_maps.push_back(
        context.get<std::unordered_map<std::string, std::size_t>>(
            "index_" + m_cfg.prefix + "_" + input));
  }
  ...
  for (const std::string &var_name : m_cfg.variableNames) {
    ...
    for (auto ientry(0); ientry < object_maps.size(); ientry++) {
      ...
      if (map->find(m_cfg.prefix + "_" + var_name) == map->end())
        throw std::runtime_error("Variable not available in collection: " +
                                 var_name);
    ...
    DrawCollection(canvas, object_to_be_drawn);
    canvas.Draw();

    std::string save_name = m_cfg.outputFolder + "/comparison_" + m_cfg.prefix +
                            "_" + var_name + ".pdf";
