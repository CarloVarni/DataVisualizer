...
namespace Algorithm {
  ...
  class RootFileProfileMakerAlgorithm
  ...
  public:
    struct Config {
      ...
      std::function<
	std::shared_ptr<EventDataModel::ProfileObjectCollection>(TFile*)
	> extractionFunction;
    };

    RootFileProfileMakerAlgorithm() = delete;
    RootFileProfileMakerAlgorithm(const std::string& name,
				  const Config& cfg);
    RootFileProfileMakerAlgorithm(const RootFileProfileMakerAlgorithm&) = delete;
    RootFileProfileMakerAlgorithm& operator=(const RootFileProfileMakerAlgorithm&) = delete;
