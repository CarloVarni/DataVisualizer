...
namespace Algorithm {
  ...
  public:
    ...
    FilteringAlgorithm(const std::string& name,
		       const Config& cfg);
  ...
  FilteringAlgorithm<data_t>::initialize() {
    ...
    if (m_cfg.inputMaskName.empty())
      throw std::invalid_argument("Input Mask Name has not beed specified for algorithm " + name());
    
    if (m_cfg.outputMaskName.empty())
      throw std::invalid_argument("Output Mask Name has not beed specified for algorithm " + name());

    if (not m_cfg.filterFunction)
      throw std::invalid_argument("Filter Function has not been specified for algorithm " + name());
  }
  ...
  FilteringAlgorithm<data_t>::execute(Core::EventContext& context) 
  {
    ...
    std::shared_ptr<std::vector<bool>> outputMask =
      std::make_shared<std::vector<bool>>(inputMask->begin(),
					  inputMask->end());
