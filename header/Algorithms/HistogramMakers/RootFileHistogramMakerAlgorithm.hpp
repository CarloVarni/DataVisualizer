...
class RootFileHistogramMakerAlgorithm : public Core::BaseAlgorithm {
public:
  struct Config {
    ...
    std::function<std::pair<
        ...
        std::shared_ptr<EventDataModel::HistogramObjectCollection_2D>>(TFile *)>
        extractionFunction;
  };

  RootFileHistogramMakerAlgorithm() = delete;
  RootFileHistogramMakerAlgorithm(const std::string &name, const Config &cfg);
  RootFileHistogramMakerAlgorithm(const RootFileHistogramMakerAlgorithm &) =
      delete;
  RootFileHistogramMakerAlgorithm &
  operator=(const RootFileHistogramMakerAlgorithm &) = delete;
