...
template <typename... supported_t> class MultiData {
public:
  using value_type = std::variant<supported_t...>;

  static_assert(
      not std::disjunction<std::is_same<std::string, supported_t>...>::value,
      "MultiData Object does not support std::string!");

  MultiData() = delete;

  MultiData(const std::vector<std::string> &labels,
            const std::vector<value_type> &data, float weight = 1.);
...
std::ostream &operator<<(std::ostream &os,
                         const MultiData<data_type...> &data) {
  os << "MultiData object with " << data.size()
     << " entries [w = " << data.weight() << "]\n";
...
MultiData<supported_t...>::MultiData(
    const std::vector<std::string> &labels,
    const std::vector<std::variant<supported_t...>> &data, float weight)
    : m_weight(weight) {
  if (labels.size() != data.size())
    throw std::runtime_error(
        "MultiData error: Size of labels and data do not match");
  ...
  for (auto i(0); i < labels.size(); i++) {
    ...
    std::visit([=](auto arg) { m_data.push_back(arg); }, data.at(i));
  }
}

template <typename... supported_t>
std::size_t MultiData<supported_t...>::size() const {
  return m_indexMap.size();
}

template <typename... supported_t>
float MultiData<supported_t...>::weight() const {
  return m_weight;
}

template <typename... supported_t>
bool MultiData<supported_t...>::hasLabel(const std::string &label) const {
  return m_indexMap.find(label) != m_indexMap.end();
}

template <typename... supported_t>
std::vector<std::string> MultiData<supported_t...>::labels() const {
...
using MultiDataObject = MultiData<double, int, float, long, unsigned int, short,
                                  bool, unsigned short>;
